...
We have since added an exploration-stall rule (eight consecutive successful steps that neither change a file nor verify anything trigger a consult); it did not fire once in the 95-instance run described next, where the small model fails by producing malformed actions rather than by reading aimlessly.
...
On 95 instances (83 Django, 8 pytest, 3 pylint, 1 Flask) with the cheaper stack above, the result does not hold.
The middle model alone (DeepSeek~V4~Flash) resolves 44 at \$0.011 per task, the ladder 37 at \$0.026, and the small model alone 10.
The 9B model produces malformed actions twice in a row on 53 of its 95 solo runs, and 51 of the ladder's 99 handoffs are for that reason; it spends two thirds of the ladder's tokens while costing as much per task as the middle model, and because the step cap is shared across rungs, the median handoff at step 8 leaves the stronger model 22 of 30 steps.
Where the ladder hands off, it resolves 28 of 69 instances against 31 for the middle model alone on the same instances; the larger loss is on instances the small model never hands off (9 of 26, against 13).
Small-model-first pays when the small model is both cheaper per task than the rung above and able to act; on repository work with this pair it is neither, and the open question becomes whether escalating from the strongest cheap model beats that model alone.
...
Three coding workloads, one small model in the main comparison, two repository runs (15 and 95 instances) of which the ladder wins only the smaller one, where its middle model was weaker, and rule-based triggers whose thresholds were set by hand.
